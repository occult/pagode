...
func (_q *ChatMessageQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*ChatMessage, error) {
	...
	if _q.withRoom != nil || _q.withSender != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := _q.withSender; query != nil {
		if err := _q.loadSender(ctx, query, nodes, nil,
			func(n *ChatMessage, e *User) { n.Edges.Sender = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *ChatMessageQuery) loadRoom(ctx context.Context, query *ChatRoomQuery, nodes []*ChatMessage, init func(*ChatMessage), assign func(*ChatMessage, *ChatRoom)) error {
...
func (_q *ChatMessageQuery) loadSender(ctx context.Context, query *UserQuery, nodes []*ChatMessage, init func(*ChatMessage), assign func(*ChatMessage, *User)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*ChatMessage)
	for i := range nodes {
