...
func (_q *SubscriptionQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Subscription, error) {
	...
	if query := _q.withCustomer; query != nil {
		if err := _q.loadCustomer(ctx, query, nodes, nil,
			func(n *Subscription, e *PaymentCustomer) { n.Edges.Customer = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *SubscriptionQuery) loadCustomer(ctx context.Context, query *PaymentCustomerQuery, nodes []*Subscription, init func(*Subscription), assign func(*Subscription, *PaymentCustomer)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Subscription)
	for i := range nodes {
		if nodes[i].payment_customer_subscriptions == nil {
			continue
		}
		fk := *nodes[i].payment_customer_subscriptions
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "payment_customer_subscriptions" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
